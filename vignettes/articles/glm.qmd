...
By default, `visreg` turns off partial residuals when a transformation has been applied to the vertical axis. The reason for this is that residuals are useful to examine on the linear predictor scale, but can be misleading when transformed --- a moderate residual can appear huge after a transformation, and vice versa.
...
Another option, if you want to show the location of the observations, but not display them in this potentially misleading way, is to use a rug, either a single rug along the bottom (`rug = 1`) or separate rugs for positive and negative residuals (`rug = 2`):
