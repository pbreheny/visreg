  ...
  for (k in 1:K) {
    ...
    if (band) {
      poly.args <- list(
        x = c(xx, rev(xx)),
        y = c(
          rep(lframe$lwr[subscripts][k], 2),
          rev(rep(lframe$upr[subscripts][k], 2))
        ),
    ...
    if (!partial) {
      lattice::panel.lines(
        xx,
        rep(lframe$fit[subscripts][k], 2),
        subscripts = subscripts,
        ...
      )
    } else {
      ind <- (lresids$by == lframe$by[subscripts][1]) &
        (lresids$x == levels(lresids$x)[k])
