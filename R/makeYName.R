makeYName <- function(fit, scale, trans, type) {
  if (
    scale == "response" |
      (class(fit)[1] %in% c("lm", "aov") & identical(trans, I))
  ) {
    if (type == "contrast") {
      yName <- as.expression(substitute(
        list(Delta) * x,
        list(x = as.character(formula(fit)[2]))
      ))
    } else {
    ...
    if (type == "contrast") {
      yName <- sapply(colnames(fit$residuals), function(y) {
    ...
    if (fit$type == "classification") {
      yName <- paste0("Pr(", as.character(formula(fit)[2]), ")")
    }
  } else {
    yName <- NULL
  }
