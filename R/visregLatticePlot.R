visregLatticePlot <- function(
  ...
  if (identical(strip.names, FALSE)) {
    strip <- lattice::strip.custom(
      strip.names = FALSE,
      factor.levels = levels(as.factor(bb)),
      strip.levels = c(TRUE, TRUE),
      fg = lattice::trellis.par.get("strip.background")$col
    )
    ...
    if (is.factor(v$fit[, v$meta$by])) {
      ...
      strip <- lattice::strip.custom(
        strip.names = FALSE,
        factor.levels = paste(v$meta$by, abbrNum(bb), sep = ": "),
        strip.levels = c(TRUE, TRUE),
        fg = lattice::trellis.par.get("strip.background")$col
      )
    }
  ...
  plot.args <- list(
    x = formula(lframe$fit ~ lframe$xx | lframe$by),
    type = "l",
    ylim = ylim,
    xlab = v$meta$x,
    ylab = ylab,
  ...
  if (is.factor(x)) {
    ...
    scales <- list(
      x = list(
        at = ((0:(K - 1)) / len + (1 - whitespace) / (2 * len)),
        labels = levels(x)
      )
    )
